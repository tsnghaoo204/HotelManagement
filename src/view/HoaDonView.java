...
public class HoaDonView extends JFrame{
	private static HoaDonView view = null;
	
	public static HoaDonView getInstance() {
		if (view == null) view = new HoaDonView();
		return view;
	}
	...
	public void Unit() {
		...
		btnXem.addActionListener(hoaDonController);
		btnThm.addActionListener(hoaDonController);
		btnTim.addActionListener(hoaDonController);
		btnInHoaDon.addActionListener(hoaDonController);
		btnQuayLai.addActionListener(hoaDonController);
	}
	...
	public void Xem(HoaDonView hoaDonView) {
		String soPhong = "";
		soPhong = textField_1.getText();
		if(soPhong.equals("")) {
			JOptionPane.showMessageDialog(hoaDonView, "Bạn phải nhập số phòng!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		Phong phong = phongDao.SelectPhongByID(soPhong);
		if(phong == null) {
			JOptionPane.showMessageDialog(hoaDonView, "Phòng chưa đặt hoặc không đúng!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			return;
		} else {
			...
			if(kh != null) {
				...
				JOptionPane.showMessageDialog(hoaDonView, "Phòng chưa được đặt!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
				return;
			}
	...
	public void ThemHoaDon(HoaDonView hoaDonView) {
		String ngayTra = "";
		...
		if(ngayTra.equals("")) {
			JOptionPane.showMessageDialog(hoaDonView, "Bạn phải nhập ngày trả!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		Phong phong = phongDao.SelectPhongByID(soPhong);
		if(phong == null) {
		...
		try {
			...
			JOptionPane.showMessageDialog(hoaDonView, "Ngày nhập phải đúng định dạnh yyyy-MM-dd HH:mm", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		KhachHang kh = khachHangDao.SelectKhachHangByID(phong.getMaKH());
		...
		if(hoaDonDao.InsertHoaDon(hd) != 0) {
			JOptionPane.showMessageDialog(hoaDonView, "Thêm hóa đơn thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			ArrayList<HoaDon> dshd = new ArrayList<HoaDon>();
			dshd = hoaDonDao.SelectAllHoaDon();
			HienThiData(dshd);
		}
	...
	public void TimKiem() {
		if(textField_1.getText().isEmpty()) {
			JOptionPane.showMessageDialog(rootPane, "Vui lòng nhập số phòng để tim kiếm!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		ArrayList<HoaDon> dshd1 = new ArrayList<HoaDon>();
		dshd1 = hoaDonDao.SelectHoaDonByIDSoPhong(textField_1.getText().trim());
		if(dshd1.size() > 0) {
			HienThiData(dshd1);
		} else {
	...
	public void InHoaDon(HoaDonView hoaDonView) {
		int r = tableHoaDon.getSelectedRow();
		if(r < 0) {
			JOptionPane.showMessageDialog(hoaDonView, "Bạn phải chọn 1 hóa đơn!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
		} else {
			String[][] duLieuTruyen = {
