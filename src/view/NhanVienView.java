...
public class NhanVienView extends JFrame {
	...
	public NhanVienView() {
		...
		MouseListener mouseListener = new QuanLyNhanVienController(this);
	...
	public void HienThiData() {
		...
		tableHoaDon.setModel(defaultTableModel);
		tableHoaDon.getColumnModel().getColumn(1).setPreferredWidth(170);
	...
	public void ThemNhanVien() {
		...
		NhanVien nv = new NhanVien(txtTen.getText().trim(), (String)(rdbGioiTinh.getSelection() == rdbNam.getModel() ? "Nam" : "Nữ").toString() ,txtDiaChi.getText(), txtSDT.getText().trim(), (String)(rdbCaLam.getSelection() == rdbSang.getModel() ? "Sáng" : "Tối"), Double.parseDouble(getNormalMonney(txtLuong.getText().trim())), Integer.parseInt(txtNghi.getText().trim()) ,(String)(rdbTinhTrang.getSelection() == rdbCoMat.getModel() ? "Có mặt" : "Vắng"), txtMa.getText().trim());
